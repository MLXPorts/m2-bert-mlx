...
Tunables (env)
- `M2_BERT_GEMM_TILE_AV`, `M2_BERT_GEMM_TILE_ATB`: override tiles (e.g., 32x8).
- `M2_BERT_GEMM_PAD=1`: add +1 column padding in shared tiles to mitigate bank
  conflict patterns.
- `M2_BERT_GEMM_ALIGN_EXECW=1`: try aligning square tile size T to
  `threadExecutionWidth` (if T*T ≤ 1024).
- (Reserved) `M2_BERT_GEMM_VEC4`, `M2_BERT_GEMM_DB`: vectorized loads/double‑buffer
  prototypes can be added under these gates if needed.

Notes
- Adapted for M2-BERT from MetalFaiss patterns; correctness validated against
...
def _select_tile_av() -> Tuple[int, int]:
    ...
    Env override: M2_BERT_GEMM_TILE_AV="TMxT" (e.g., 32x8).
    Defaults to (16, 16).
    """
    env = os.environ.get("M2_BERT_GEMM_TILE_AV") or os.environ.get("M2_BERT_GEMM_TILE")
    if env:
        try:
...
def _select_tile_atb() -> Tuple[int, int, int]:
    ...
    Env override: M2_BERT_GEMM_TILE_ATB="TNxTK"; TI fixed at 16.
    Defaults to (16, 16, 16).
    """
    env = os.environ.get("M2_BERT_GEMM_TILE_ATB")
    if env:
        try:
...
def _format_av_source_square(T: int) -> str:
    ...
    PAD = 1 if os.environ.get("M2_BERT_GEMM_PAD", "0") == "1" else 0
    tpl = Template(r"""
...
def _build_av_kernel():
    ...
    TM, Tsel = _TILES_AV
    T = int(min(TM, Tsel))
    use_db = os.environ.get("M2_BERT_GEMM_DB", "0") == "1"
