...
Run from project root:
  PYTHONPATH=. python -m bert.src.blockdiag.test_gemm_kernels

Set M2_BERT_GEMM_NAIVE=1 to force naive path.
"""
import os
import time
...
def check(shape=(64, 128, 32)):
    ...
    print(f"shape={shape} :: AV diff={diff_av:.3e} ({t1:.4f}s vs {t0:.4f}s), AT_B diff={diff_atb:.3e} ({t3:.4f}s vs {t2:.4f}s)")


def main():
    """The main function of the script."""
    naive = os.environ.get("M2_BERT_GEMM_NAIVE", "0") == "1"
