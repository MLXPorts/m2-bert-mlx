#!/usr/bin/env python
"""
TensorLint: A comprehensive linting tool for tensor operation codebase.
...
It helps ensure that tensor operation code remains pure, efficient, and maintainable.
"""

# Set this to False to force TensorLint to always check all issues
...
class PrecisionReducingVisitor(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        self.mlx_aliases = set()
        self.torch_np_pi_usage = []  # Using np.pi in torch files
    ...
    def visit_Import(self, node):
        ...
        for name in node.names:
            ...
            elif name.name == 'torch':
                self.backend_imports.add(f"import {name.name}")
                self.torch_aliases.add(name.asname or 'torch')
            elif name.name == 'mlx':
                self.backend_imports.add(f"import {name.name}")
                self.mlx_aliases.add(name.asname or 'mlx')
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module == 'numpy':
            for name in node.names:
                ...
                self.numpy_aliases.add(name.asname or name.name)
        elif node.module == 'torch' or (node.module and node.module.startswith('torch.')):
            filename = getattr(node, 'filename', '')
            for name in node.names:
                self.backend_imports.add(f"from {node.module} import {name.name}")
                self.torch_aliases.add(name.asname or name.name)
        elif node.module == 'mlx' or (node.module and node.module.startswith('mlx.')):
            filename = getattr(node, 'filename', '')
            for name in node.names:
                self.backend_imports.add(f"from {node.module} import {name.name}")
                self.mlx_aliases.add(name.asname or name.name)
        self.generic_visit(node)
    ...
    def visit_Attribute(self, node):
        """Visit attribute access to detect np.pi usage in torch files."""
        # Check for np.pi when torch is imported
        if isinstance(node.value, ast.Name):
            if node.value.id in self.numpy_aliases and node.attr == 'pi':
                if self.torch_aliases:  # If torch is imported in this file
                    location = f"{self.current_function}:{node.lineno}" if self.current_function else f"line {node.lineno}"
                    self.torch_np_pi_usage.append({
                        'usage': f"{node.value.id}.pi",
                        'location': location,
                        'line': node.lineno
                    })
        self.generic_visit(node)
...
def check_ast_for_issues(file_path: str) -> Tuple[bool, List[str], List[str], List[Dict], List[Dict], List[Dict], List[Tuple[str, int]], bool, List[str], List[Dict], List[Dict]]:
    """Use AST to check for NumPy imports, usage, precision-reducing casts, tensor conversions, Python operators, unused imports, and torch-specific issues."""
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
...
BACKEND_TENSOR_PATHS = {
    "numpy": ["backend/numpy/tensor"],
    "torch": ["backend/torch/tensor"],
    "mlx": ["backend/mlx/tensor"]
}

# Define frontend tensor paths for the new folder structure
FRONTEND_TENSOR_PATHS = {
    "common": ["nn/tensor/common"]
}
...
def analyze_file(file_path: str) -> Dict:
    ...
    ast_has_numpy, ast_numpy_imports, ast_numpy_usages, precision_casts, tensor_conversions, python_operators, unused_imports, ast_has_backend_imports, ast_backend_imports, backend_usage, torch_np_pi_usage = check_ast_for_issues(file_path)
    ...
    return {
        ...
        "torch_np_pi_usage": torch_np_pi_usage
    }
...
def print_results(results: List[Dict], verbose: bool = False, show_all: bool = True,
    ...
    files_with_torch_np_pi = [result for result in results if result["torch_np_pi_usage"]]
    ...
    if len(results) > 0:
        ...
        if show_all:
            print(f"Files using np.pi in torch code: {len(files_with_torch_np_pi)} ({len(files_with_torch_np_pi)/len(results)*100:.2f}%)")
    ...
    if verbose:
        ...
        if show_all and files_with_torch_np_pi:
            print("\nFiles using np.pi in torch code:")
            for result in files_with_torch_np_pi:
                print(f"\n{result['file']}:")
                for usage in result["torch_np_pi_usage"]:
                    print(f"  {usage['usage']} at {usage['location']}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="TensorLint: A comprehensive linting tool for tensor operation codebase.")
    ...
    for path in args.paths:
        if os.path.isfile(path) and path.endswith('.py'):
            ...
            # Analyze a directory
            dir_results = analyze_directory(path, args.exclude)
